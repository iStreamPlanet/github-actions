...
#### [`wait-for-commit`](reusable-workflows/wait-for-commit)

Reusable workflow that can pause execution of the parent workflow while `status checks` complete and facilitates the addition of notifications for failed `status checks` to our repositories.
